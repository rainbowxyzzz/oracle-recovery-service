...
- 当前平台登录者均为超级管理员，不设置普通 SQL 开发用户；平台外部的 Doris 下游普通账号不通过本工作台执行 SQL，只由 Doris 数据库授权限制为访问安全对象。
...
6. 已确认：当前平台登录者均为超级管理员，可按既有权限执行 Doris DDL；平台外部的 Doris 下游账号不使用本工作台。
7. 编排画布是否直接复用现有离线开发，还是新增独立 ETL 画布？
8. 调度失败后是否需要告警，例如页面通知、邮件、接口回调？

## 21. 2026-09-04 SQL 集合与生产版本治理

### 21.1 背景与目标

Doris SQL 开发工作台现有“已保存 SQL 任务”以独立 `doris_sql` 组件任务保存。任务较多后缺少业务集合、固定执行顺序和集合级生产版本，导致人工查找困难，数据自动化、Harness、调度和血缘只能引用分散对象。

本轮将工作台升级为“SQL 集合 + SQL 任务 + 开发版 + 生产版 + 集合运行”的治理方式。为避免建立第二套编排和调度体系，SQL 集合复用现有 `DataPlatformWorkflow`，集合成员复用现有 `DataPlatformNode(node_type=doris_sql)`，开发版、生产版、运行和调度继续复用 `DataPlatformWorkflowVersion` 与 `DataPlatformWorkflowRun`。

### 21.2 业务规则

1. SQL 集合拥有名称、说明、业务域、数据层级、标签、默认 Doris 连接和默认数据库；集合名称不替代 SQL 任务名称。
2. 一个集合可以选择多个现有 Doris SQL 任务并调整顺序。同一任务在同一集合中只能出现一次，但允许被多个集合复用。
3. 集合成员顺序转换为串行依赖，集合运行按顺序执行；任一 SQL 失败后停止后续节点并保留逐节点日志。
4. 集合保存更新开发版，并重新冻结当时所选 SQL 任务的最新修订；后续单独修改 SQL 任务不会静默改变已发布生产版本。
5. “测试运行”执行当前开发版；“发布”从当前开发版生成不可变生产版本并上线；“运行生产版”只执行当前在线生产版本。
6. 数据自动化、Harness、调度和其他模块引用生产版本 ID，不引用可变开发版或任务名称。集合详情展示已发现的流水线引用和调度状态。
7. 未加入任何集合的历史或新建 Doris SQL 任务统一展示为“未分组 SQL”；不自动迁移、不复制、不改名，也不改变其原有单任务保存、运行和删除语义。
8. 归档集合会停用其在线版本与未来调度，但不删除 SQL 任务、生产快照或历史运行记录。
9. SQL 集合不允许混入数据同步、SM3、SM4 或变化触发节点；跨类型编排仍在离线开发模块完成。

### 21.3 页面与接口

Doris SQL 开发工作台新增“SQL 集合”管理区，至少提供：集合列表、未分组数量、集合基本信息、成员多选、上移/下移、保存、测试运行、发布、运行生产版、归档、版本状态、最近运行和引用关系。

2026-09-07 页面表现固定为视口内的宽工作台弹窗：左侧为可搜索的集合列表，右侧使用“集合配置 / SQL 编排 / 运行记录”页签；底部固定保留归档、开发测试、生产发布、生产运行和保存操作。该调整只改变信息架构和可见性，不改变字段、接口、权限、成员顺序、版本、运行或归档语义。

集合接口挂在 `/api/v1/doris-sql-etl/collections`，读取要求 `dorisSqlEtl:read`；集合维护要求数据平台设计能力；发布要求 `dataPlatform:publish`；运行要求 `dataPlatform:execute`；运行记录读取要求 `dataPlatform:monitor`。后端仍执行现有 SQL 安全校验、审计和队列规则。

### 21.4 兼容性与风险边界

- 不新增并行的 SQL 执行器、调度器或版本表，不改变现有数据平台工作流表结构。
- 不改变既有 Doris SQL 任务 API、节点配置、任务修订号、离线工作流、数据自动化流水线或 Harness 计划契约。
- 生产版本继续冻结组件任务快照；发布后的任务修改必须重新保存集合并重新发布才生效。
- 集合运行可能包含 DDL/DML，页面必须明确区分开发测试与生产运行，继续遵守现有危险 SQL 权限和审计规则。

### 21.5 验收标准

1. 可以创建集合、保存中文名称和元数据，刷新后正确回填。
2. 可以把至少三个已保存 SQL 任务加入集合、排序并回读，重复成员被拒绝。
3. 开发测试按保存顺序运行并产生集合及逐节点运行记录。
4. 发布后产生不可变在线生产版本；修改原 SQL 任务不改变旧生产快照，重新发布后形成新版本。
5. 生产版可以被数据自动化/Harness 按现有版本 ID 引用，集合详情能展示引用该版本的流水线。
6. 归档集合不会删除成员 SQL 任务和历史运行；旧单任务仍可新增、编辑、运行和删除。
7. 页面在最大化、左右半屏和还原窗口下无关键按钮遮挡、页面级横向溢出或非预期控制台错误。
